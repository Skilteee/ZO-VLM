...
def smooth_and_quant_temporary(model, w_train_bits, a_train_bits, isllama):
    # if args.let:
    # with torch.no_grad():
    ...
    if isllama:
        ...
        if a_train_bits == 16:
            ...
            # model.mlp.down_proj.temp_weight = model.mlp.down_proj.weight.data


    else:
        model.self_attn.out_proj.temp_weight = model.self_attn.out_proj.weight.detach().clone()
        model.self_attn.out_proj.temp_bias = model.self_attn.out_proj.bias.detach().clone()
        model.fc2.temp_weight = model.fc2.weight.detach().clone()
        model.fc2.temp_bias = model.fc2.bias.detach().clone()

        if a_train_bits == 16:
            model.self_attn.q_proj.temp_weight = model.self_attn.q_proj.weight.detach().clone()
            model.self_attn.q_proj.temp_bias = model.self_attn.q_proj.bias.detach().clone()
            model.self_attn.k_proj.temp_weight = model.self_attn.k_proj.weight.detach().clone()
            model.self_attn.k_proj.temp_bias = model.self_attn.k_proj.bias.detach().clone()
            model.self_attn.v_proj.temp_weight = model.self_attn.v_proj.weight.detach().clone()
            model.self_attn.v_proj.temp_bias = model.self_attn.v_proj.bias.detach().clone()
            model.self_attn_layer_norm.temp_weight = model.self_attn_layer_norm.weight.detach().clone()
            model.self_attn_layer_norm.temp_bias = model.self_attn_layer_norm.bias.detach().clone()

            model.final_layer_norm.temp_weight = model.final_layer_norm.weight.detach().clone()
            model.final_layer_norm.temp_bias = model.final_layer_norm.bias.detach().clone()
            model.fc1.temp_weight = model.fc1.weight.detach().clone()
            model.fc1.temp_bias = model.fc1.bias.detach().clone()
    ...
    for name, module in model.named_modules():
        if isinstance(module, QuantLinear):
            if hasattr(module, "temp_weight"):
                module.temp_weight.data = module.weight_quantizer(module.temp_weight.data)
            else:
...
def use_parameters(model):
    for name, module in model.named_modules():
        if isinstance(module, QuantLinear) or isinstance(module, OmniLlamaRMSNorm):
...
def smooth_and_quant_inplace(model, args, isllama):
    if args.let:
        ...
        if isllama:
            ...
            smooth_ln_fcs_inplace(model.self_attn_layer_norm,
                                  [model.self_attn.q_proj, model.self_attn.k_proj, model.self_attn.v_proj],
                                  model.qkv_smooth_scale, model.qkv_smooth_shift)
